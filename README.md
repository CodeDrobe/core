...
Compatibility is judged per renderer window: secondary windows missing `required` landmarks (popped-out chats, hidden overlay surfaces) are demoted to `"skipped": true` and reported, and probe, apply, and verify fail only when no window qualifies. Verify never skips a window the theme is installed on, so a regression on a themed window still fails. Errors name the failing window and report the scope, active context, name, attempted selectors, selector parse errors, and the adapter's last-verified versions. Missing `recommended` landmarks are returned as warnings without blocking injection. Context checks run only while one of their `when.any` landmarks is visible. Pages that have not rendered their root landmark yet (splash and loading screens) are treated as booting and re-probed for the full apply budget instead of failing fast.
